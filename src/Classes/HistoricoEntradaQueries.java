 ...
 * @author Rafael
 */
public class HistoricoEntradaQueries {
    ...
    public HistoricoEntradaQueries() {
        try {
            connection = MysqlConnect.connectDB();
            selectAllCodigos = connection.prepareStatement("SELECT cod FROM notaentrada");
            deleteNota = connection.prepareStatement("DELETE from notaprodutoentrada "
                    + "WHERE codNotaEntrada = ?");
            deleteNotaFinal = connection.prepareStatement("DELETE from notaentrada WHERE cod = ?");
            selectProdutoHistorico = connection.prepareStatement("SELECT *from notaprodutoentrada WHERE codNotaEntrada = ?");
            ...
            selectTabelaVisualizar = connection.prepareStatement("SELECT quantidade,unidade,descricao,valor,valor*quantidade "
                    + "FROM notaprodutoentrada,produto WHERE codProdutoEntrada = codProd AND codNotaEntrada = ? AND codProd = ?");
        } catch (SQLException ex) {
            Logger.getLogger(HistoricoEntradaQueries.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
        ...
        try (ResultSet resultSet = selectProdutoHistorico.executeQuery()) {
            ...
            while (resultSet.next()) {
                ...
                results.add(new NotaProduto(
                        ...
                        resultSet.getInt("codNotaEntrada"),
                        resultSet.getInt("codProdutoEntrada")
                ));
            }
        } catch (SQLException sqle) {
            JOptionPane.showMessageDialog(null, sqle);
