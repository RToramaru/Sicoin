 ...
 * @author Rafael
 */
public class NotaSaidaTabelaQueries {
    ...
    public NotaSaidaTabelaQueries() {
        try {
            
            connection = MysqlConnect.connectDB();
            
            insertTabela = connection.prepareStatement("INSERT INTO notaprodutosaida "
                    + "(quantidade,codNotaSaida,codProdutoSaida,preconotaSaida) VALUES(?,?,?,?)");
            
            selectTabela = connection.prepareStatement("SELECT * FROM produto WHERE descricao = ?");
            
            deleteTabela = connection.prepareStatement("DELETE FROM notaprodutosaida "
                    + "WHERE codNotaSaida = ? AND descricao = ? AND quantidade = ?");
    ...
    public int addTabelaSaida(float quantidade,int codNotaSaida, int codProdutoSaida, float precoNotaSaida){
        int result = 0;
        try{
            insertTabela.setFloat(1, quantidade);
            insertTabela.setInt(2, codNotaSaida);
            insertTabela.setInt(3, codProdutoSaida);
            insertTabela.setFloat(4, precoNotaSaida);
    ...
    public int deleteNotaTabela(int codNotaSaida,String descricao,float quantidade) throws SQLException{
        int result = 0;
        deleteTabela.setInt(1, codNotaSaida);
